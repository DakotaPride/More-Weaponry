...
public class BardSummoningBlock
        extends Block {
     ...
     */
    public BardSummoningBlock(Block regularBlock, Settings settings) {
        super(settings);
        this.regularBlock = regularBlock;
        REGULAR_TO_INFESTED_BLOCK.put(regularBlock, this);
    }

    public BardSummoningBlock(Settings settings, Block regularBlock) {
        super(settings);
        this.regularBlock = regularBlock;
    }
    ...
    private void spawnBard(ServerWorld world, BlockPos pos) {
        BardEntity bardEntity = MoreWeaponryEntities.BARD.create(world);
        assert bardEntity != null;
        bardEntity.refreshPositionAndAngles((double)pos.getX() + 0.5, pos.getY(), (double)pos.getZ() + 0.5, 0.0f, 0.0f);
    ...
    public void onDestroyedByExplosion(World world, BlockPos pos, Explosion explosion) {
        if (world instanceof ServerWorld) {
            this.spawnBard((ServerWorld)world, pos);
        }
    }
