...
public class SickenedSummoningBlock
        extends Block {
     ...
     */
    public SickenedSummoningBlock(Block regularBlock, Settings settings) {
        super(settings);
        this.regularBlock = regularBlock;
        REGULAR_TO_INFESTED_BLOCK.put(regularBlock, this);
    }

    public SickenedSummoningBlock(Settings settings, Block regularBlock) {
        super(settings);
        this.regularBlock = regularBlock;
    }
    ...
    private void spawnSickened(ServerWorld world, BlockPos pos) {
        SickenedEntity sickenedEntity = MoreWeaponryEntities.SICKENED.create(world);
        assert sickenedEntity != null;
        sickenedEntity.refreshPositionAndAngles((double)pos.getX() + 0.5, pos.getY(), (double)pos.getZ() + 0.5, 0.0f, 0.0f);
    ...
    public void onDestroyedByExplosion(World world, BlockPos pos, Explosion explosion) {
        if (world instanceof ServerWorld) {
            this.spawnSickened((ServerWorld)world, pos);
        }
    }
