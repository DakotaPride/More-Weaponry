...
public class NewSickenedGravesStructure extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final Codec<NewSickenedGravesStructure> CODEC = RecordCodecBuilder.<NewSickenedGravesStructure>mapCodec(instance ->
            instance.group(NewSickenedGravesStructure.configCodecBuilder(instance),
            ...
            ).apply(instance, NewSickenedGravesStructure::new)).codec();
    ...
    public NewSickenedGravesStructure(Config config,
    ...
    public Optional<StructurePosition> getStructurePosition(Context context) {
        ...
        if (!NewSickenedGravesStructure.extraSpawningChecks(context)) {
    ...
    public StructureType<?> getType() {
        return MoreWeaponry.SICKENED_GRAVES; // Helps the game know how to turn this structure back to json to save to chunks
    }
}
