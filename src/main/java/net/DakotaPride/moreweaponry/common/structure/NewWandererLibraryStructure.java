...
public class NewWandererLibraryStructure extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final Codec<NewWandererLibraryStructure> CODEC = RecordCodecBuilder.<NewWandererLibraryStructure>mapCodec(instance ->
            instance.group(NewWandererLibraryStructure.configCodecBuilder(instance),
            ...
            ).apply(instance, NewWandererLibraryStructure::new)).codec();
    ...
    public NewWandererLibraryStructure(Config config,
    ...
    public Optional<StructurePosition> getStructurePosition(Context context) {
        ...
        if (!NewWandererLibraryStructure.extraSpawningChecks(context)) {
    ...
    public StructureType<?> getType() {
        return MoreWeaponry.WANDERER_LIBRARY; // Helps the game know how to turn this structure back to json to save to chunks
    }
}
